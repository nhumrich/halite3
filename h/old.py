...
def run():
    ...
    while True:
        ...
        for ship in ship_queue:
            ...
            elif (len(ship_queue) > 15 and
                  game_map.calculate_distance(ship.position, closest_dropoff(ship)) > 15 and
            ...
            # can it move?
                ...
                ship.returning = True
            elif (ship.halite_amount + game_map[ship.position].halite_amount > 900):
                # will staying make me cross over the line?
                ship.staying = True
                move(ship.position, ship.position, ship, positions_used, command_queue)
            elif (ship.halite_amount + game_map[ship.position].halite_amount > 1000):
